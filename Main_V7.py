...
class ProcessingThread(QtCore.QThread):
    resultReady = QtCore.pyqtSignal(object, object, object)  
    # emits: (frameYOLO, frameKey, keypoints3D)

    def __init__(self, parent, fps=40, threshold=0.5):
    ...
    def run(self):
        ...
        while self.running and not self.parent.closeFlag:
            t0 = time.perf_counter()

            # 取一份当前帧的“快照”，避免和采集线程冲突
            frame = [np.copy(self.parent.frame[0]), np.copy(self.parent.frame[1])]

            # 没有帧就跳过
            if frame[0] is None or frame[1] is None or (not frame[0].any()) or (not frame[1].any()):
                time.sleep(0.005)
                continue

            # === 跑你原来的处理逻辑（不要直接改 ML_V6）===
            frameYOLO, keypoints, frame_height, Cam_origin = ML.skeletonDetection(
                frame, self.threshold, self.parent.model
            )
...
class Ui_MainWindow(QMainWindow):
    ...
    def initialVariables(self):
        ...
        self.fps =50                                          # Frames per second, this is define by the camera specification
        ...
        # change to GPU
        self.model = YOLO(YOLO_models[modelSize] + ".pt")               # Load the YOLO model
        ...
        self.processFPS = 40                                  # Change the frecuency of the process live video
        ...
        self.timeFeaturesFall = []                              # Array to store the time features for the fall risk model application
         ...
         # ---- latest results cache (thread -> UI) ----
        self.latest_frameYOLO = np.copy(self.frameYOLO)
        self.latest_frameKey  = np.copy(self.frameKey)
        self.latest_keypoints3D = None
        ...
        if(self.BtnStart.text() == 'Preview'):                                                                  # If the button text is "Preview"
            ...
            with contextlib.ExitStack() as stack:                                                               # Creates a stack to manage the resources used by the cameras
                ...
                if len(device_infos) == 0:                                                                      # If there are no devices available
                    ...
                    if not hasattr(self, "procThread") or self.procThread is None:
                        self.procThread = ProcessingThread(self, fps=self.processFPS, threshold=0.5)
                        self.procThread.resultReady.connect(self.onResultReady)
                    ...
                    while(not self.closeFlag):                                                                  # Generates a continuous loop to grab new frames from the cameras
                        for mxid, q in devices.items():                                                         # Checks the output queues of each device
                            ...
                            try:
                                ...
                                print("There was a connectivity error\n {}".format(e))
                                self.commFlag = False
                                in_crop = None
    ...
    def snap(self):
        ...
        if(self.frame[0].any() or self.frame[1].any()):                         # Checks if the frames from the cameras are not empty
            ...
            elif(self.videoFlag == 1):
                PixmapA = self.imageFormat(self.latest_frameKey[0])
                PixmapB = self.imageFormat(self.latest_frameKey[1])
            else:
                PixmapA = self.imageFormat(self.latest_frameYOLO[0])
                PixmapB = self.imageFormat(self.latest_frameYOLO[1])
    ...
    def closeEvent(self, event):
        ...
        # 用户确认退出
        self.closeFlag = 1  # 让你的 while 循环跳出

        # ✅ 停掉推理线程（如果存在）
        if hasattr(self, "procThread") and self.procThread is not None:
            self.procThread.stop()     # 你在线程里自己实现 stop()
            self.procThread.wait()     # 等线程干净退出
